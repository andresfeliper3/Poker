		...
		try {
			...
			if (ronda == 0 && contadorTurnos < TOTAL_JUGADORES) {
				...
				while (idJugador != turno && !reiniciado) {
					...
					esperarTurno.await();
					System.out.println(nombreJugador+", Despertó en la ronda de apuestas");
				}
			...
			// Si estamos en la ronda de igualación de apuestas.
				...
				while (idJugador != turno && !reiniciado) {
					System.out.println("En igualación " + nombreJugador + " intenta entrar y es mandado a esperar turno en la ronda igualación");
					esperarIgualacion.await();
					System.out.println(nombreJugador+", Despertó en la ronda de igualación");
				}
				if (!estadoJugador && !reiniciado) {
					int apuesta = calcularApuesta(idJugador, operacion);
					apuestaTotal +=apuesta;
			...
			// Ronda Descartes
				...
				while (idJugador != turno) {
					...
					System.out.println(nombreJugador+", Despertó en la ronda de descartes");
				}
